...
def document_verification(request):
    try:
        ...
        if selfie_image and personal_card_image and document_name:
            ...
            if results[0] == True:
                return JsonResponse({"message": "Face matched","is_matched": str(results[0]), "matched_probability": probability[0], "is_error": "False"})
            return JsonResponse({"message":"Face didn't matched","is_matched": str(results[0]), "matched_probability": probability[0], "is_error": "True"})
